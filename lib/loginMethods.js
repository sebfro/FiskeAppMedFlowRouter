...
export function register(email, pass, pass2, fName, lName, pNr){
    ...
    if(fName !== "" && lName !== "" && pNr !== "" && pass === pass2){
        ...
        /*
        remoteCreateUser.call("registerUser", user, pass2);
        if (Meteor.userId) {
            FlowRouter.go('/homepage');
        }
        */
    ...
    Meteor.loginWithPassword(email, password, function (err) {
        ...
        if (err) {
            ...
            if(Meteor.isCordova){
                ...
                navigator.notification.alert(err.reason, null, "Feilmelding", 'OK');
            }

            if(err.reason === "Incorrect password"){
                alert(i18n.__("common.loginform.incorrectPass"));
            } else if(err.reason === "You have entered an invalid email address!"){
                alert(i18n.__("common.loginform.invalidEmail"));
                this.vMail = false;
            } else if(err.reason === "User not found" && vMail){
