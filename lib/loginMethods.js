...
export function validatePhoneNr(pNr){
    return /^\d{8}$/.test(pNr) ? null : 'error';
}
...
export function register(email, pass, pass2, fName, lName, pNr){
    ...
    return ((validateName(lName) || validateName(fName) || validateEmail(email) || validatePhoneNr(pNr) || validatePass(pass)) === null && passMatch(pass, pass2));
}
