...
if (Meteor.isServer) {
    ...
    Meteor.publish('reports.adminPageList', function reportsPublication(validated){
        return Reports.find({isValidated: validated}, { sort: { createdAt: -1}});
    });

    Meteor.publish('reports.adminPageListWithCategory', function reportsPublication(category, validated){
        return Reports.find({isValidated: validated, category: category}, { sort: { createdAt: -1}});
    });
